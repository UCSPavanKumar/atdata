...
def main():
    ...
    file_format = "s3://ascend-oeidp-batch-pinning-input-074628058490-us-east-1/AtData/supertrace_atdata/atdata_rawfiles/experian_ad_fullbuild_202407_01.txt.gz"
    df_limit          = spark.read.format("csv").option("delimiter", "\t").option("inferSchema", "false").option("header", "false").load(file_format)
    ...
    valid_df = valid_df.withColumn("concat", upper(concat_ws('\t', *hash_cols))).withColumn('hashkey', md5('concat')).withColumn("full_zip",concat(col("zip5"), col("zip4")))
